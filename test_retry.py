...
BEGIN_TIME = '2017-11-29 00:00:00'


def get_day_real(days=0, now_date=None):
    if not now_date:
...
def get_start_days(days=0, now=None):
    if not now:
        now = datetime.now()
    now = now + timedelta(days=days)
    begin_date = datetime.strptime(BEGIN_TIME, TIME_PATTERN)
    delta = (now - begin_date).days + 1
    return delta


print get_start_days(days=1)
