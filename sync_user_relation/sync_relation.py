...
from sync_user_relation.config import pg_shard, split_list, USER_RELATION_INFO
...
def get_all_block_data():
    ...
    block_datas = []
    while True:
        query_sql = 'select uid,tuid,update_time as create_time from pw_block order by uid,tuid limit {limit} offset(offset)'.format(
        ...
        if not part_data:
            break
        block_datas.extend(part_data)
        offset += limit
    return block_datas


def get_all_contact_data():
    offset = 0
    limit = 10000

    contact_datas = []
    while True:
        query_sql = 'select uid,tuid,create_time from pw_contact order by uid,tuid limit {limit} offset(offset)'.format(
            limit=limit, offset=offset)
        part_data = []
        for cur in PG_CONNS.itervalues():
            cur.execute(query_sql)
        ...
        if not part_data:
            break
        contact_datas.extend(part_data)
        offset += limit
...
def get_all_contact_req_data():
    ...
    contact_req_datas = []
    while True:
        query_sql = 'select uid,tuid,state,update_time as create_time from pw_contact_request where state!=1 order by uid,tuid limit {limit} offset(offset)'.format(
            limit=limit, offset=offset)
        part_data = []
        for cur in PG_CONNS.itervalues():
            cur.execute(query_sql)
...
def get_all_shard_data():
    ...
    shard_datas = {}
    while True:
        query_sql = 'select shard_key,shard_id from pw_user_shard  order by shard_key limit {limit} offset(offset)'.format(
            limit=limit, offset=offset)
        part_data = []
        for cur in PG_CONNS.itervalues():
            cur.execute(query_sql)
